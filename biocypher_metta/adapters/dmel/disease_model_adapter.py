    ...
    def get_edges(self):      
        ...
        for row in rows:
            ...
            if self.label == 'modelled_to_human_disease':
                source = row[0]                        
                #print(f'{props}')
                yield source, f'REJUVEBIO:dmel_disease_model_{id}', self.label, props
            elif self.label == 'modelled_to_do_term':
                source = row[0]
                target = str(row[4]).replace(':', '_').lower()   # @todo Ids should be  kept in their original format in the atomspace, not lowercased...
                props['disease_model_id'] = f'REJUVEBIO:dmel_disease_model_{id}'
